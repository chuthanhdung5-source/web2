...
export default function TimetableGrid({
  ...
  return (
    <div style={{ overflowX: 'auto', borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface)' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900, textAlign: 'center' }}>
        <thead>
          <tr style={{ background: 'var(--surface-hover)', borderBottom: '2px solid var(--border)' }}>
            <th style={{ padding: '12px 8px', width: 100, fontSize: '0.85rem' }}>Tiết / Giờ</th>
            {DAYS.map(d => (
              <th key={d.num} style={{ padding: '12px 8px', fontSize: '0.9rem', width: '13%' }}>
                {d.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {PERIODS.map((p, pIndex) => {
            const isMorningStart = p.num === 1
            const isAfternoonStart = p.num === 7
            ...
            return (
              <tr key={p.num} style={{
                ...
                <td style={{
                  ...
                  <div style={{ fontSize: '0.7rem', color: 'var(--primary-light)' }}>{p.time}</div>
                </td>
                ...
                {DAYS.map(d => {
                  ...
                  if (!session) {
                    return (
                      <td key={d.num} style={{ borderRight: '1px solid var(--border)', padding: 4, background: 'rgba(0,0,0,0.05)' }}>
                        <div style={{ height: 40, opacity: 0.2 }}>—</div>
                      </td>
                    )
                  }
                      ...
                      >
                        <div>
                          <div className="flex flex-between" style={{ marginBottom: 4 }}>
                            ...
                            <span className={`badge ${statusBadge.cls}`} style={{ fontSize: '0.65rem', padding: '2px 6px' }}>
                          ...
                          {member ? (
                            ...
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontStyle: 'italic', marginTop: 4 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              {session.status === 'registered' && (
                                <div className="flex gap-1">
                                  <button className="btn btn-success btn-sm" style={{ padding: '2px 6px', fontSize: '0.7rem', flex: 1 }}
                                    onClick={() => onApprove && onApprove(session.id, true)}>
                                    ✅ Duyệt
                                  </button>
                                  <button className="btn btn-danger btn-sm" style={{ padding: '2px 6px', fontSize: '0.7rem', flex: 1 }}
                              ...
                              <div style={{ marginTop: 4 }}>
                                ...
                                >
                                  <option value="">-- Phân công người --</option>
                                  {members.map(m => (
                            ...
                            <div>
                              {session.status === 'open' && (
                                <button
                                  ...
                                  style={{ width: '100%', padding: '4px 6px', fontSize: '0.75rem', justifyContent: 'center' }}
